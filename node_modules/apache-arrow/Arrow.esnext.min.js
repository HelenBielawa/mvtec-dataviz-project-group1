...
var eb=Symbol.toStringTag,fb,gb=B.prototype;gb.isSigned=null;gb.bitWidth=null;fb=gb[Symbol.toStringTag]="Int";B[eb]=fb;class hb extends B{constructor(){super(!0,8)}get ArrayType(){return Int8Array}}class ib extends B{constructor(){super(!0,16)}get ArrayType(){return Int16Array}}class jb extends B{constructor(){super(!0,32)}get ArrayType(){return Int32Array}}class lb extends B{constructor(){super(!0,64)}get ArrayType(){return BigInt64Array}}
class mb extends B{constructor(){super(!1,8)}get ArrayType(){return Uint8Array}}class nb extends B{constructor(){super(!1,16)}get ArrayType(){return Uint16Array}}class ob extends B{constructor(){super(!1,32)}get ArrayType(){return Uint32Array}}class pb extends B{constructor(){super(!1,64)}get ArrayType(){return BigUint64Array}}Object.defineProperty(hb.prototype,"ArrayType",{value:Int8Array});Object.defineProperty(ib.prototype,"ArrayType",{value:Int16Array});
Object.defineProperty(jb.prototype,"ArrayType",{value:Int32Array});Object.defineProperty(lb.prototype,"ArrayType",{value:BigInt64Array});Object.defineProperty(mb.prototype,"ArrayType",{value:Uint8Array});Object.defineProperty(nb.prototype,"ArrayType",{value:Uint16Array});Object.defineProperty(ob.prototype,"ArrayType",{value:Uint32Array});Object.defineProperty(pb.prototype,"ArrayType",{value:BigUint64Array});
...
class Vb extends A{constructor(a,b){super();this.unit=a;this.bitWidth=b}get typeId(){return y.Time}toString(){return`Time${this.bitWidth}<${v[this.unit]}>`}get ArrayType(){switch(this.bitWidth){case 32:return Int32Array;case 64:return BigInt64Array}throw Error(`Unrecognized ${this[Symbol.toStringTag]} type`);}}var Wb=Symbol.toStringTag,Xb,Yb=Vb.prototype;Yb.unit=null;Yb.bitWidth=null;Xb=Yb[Symbol.toStringTag]="Time";Vb[Wb]=Xb;class Zb extends Vb{constructor(){super(v.SECOND,32)}}
...
b*c);default:return a.reduce((e,{values:f})=>{e.pb.set(f,e.offset);e.offset+=f.length;return e},{pb:new d(b*c),offset:0}).pb}}return[...this]}toString(){return`[${[...this].join()}]`}getChild(a){return this.getChildAt(this.type.children?.findIndex(b=>b.name===a))}getChildAt(a){return-1<a&&a<this.numChildren?new F(this.data.map(({children:b})=>b[a])):null}get isMemoized(){return A.isDictionary(this.type)?this.data[0].dictionary.isMemoized:!1}memoize(){if(A.isDictionary(this.type)){const a=new af(this.data[0].dictionary),
b=this.data.map(c=>{c=c.clone();c.dictionary=a;return c});return new F(b)}return new af(this)}unmemoize(){if(A.isDictionary(this.type)&&this.isMemoized){const a=this.data[0].dictionary.unmemoize(),b=this.data.map(c=>{c=c.clone();c.dictionary=a;return c});return new F(b)}return this}}
...
if(a instanceof Int32Array)return new F([I({...b,type:new jb})]);if(a instanceof BigInt64Array)return new F([I({...b,type:new lb})]);if(a instanceof Uint8Array||a instanceof Uint8ClampedArray)return new F([I({...b,type:new mb})]);if(a instanceof Uint16Array)return new F([I({...b,type:new nb})]);if(a instanceof Uint32Array)return new F([I({...b,type:new ob})]);if(a instanceof BigUint64Array)return new F([I({...b,type:new pb})]);if(a instanceof Float32Array)return new F([I({...b,type:new vb})]);if(a instanceof
Float64Array)return new F([I({...b,type:new wb})])}}throw Error("Unrecognized input");};function cf(a){if(!a||0>=a.length)return function(){return!0};let b="";const c=a.filter(d=>d===d);0<c.length&&(b=`
    switch (x) {${c.map(d=>`
        case ${"bigint"!==typeof d?Oa(d):da?`${Oa(d)}n`:`"${Oa(d)}"`}:`).join("")}
            return false;
    }`);a.length!==c.length&&(b=`if (x !== x) return false;\n${b}`);return new Function("x",`${b}\nreturn true;`)};const df=(a,b)=>(a*b+63&-64||64)/b,ef=(a,b=0)=>a.length>=b?a.subarray(0,b):ma(new a.constructor(b),a,0);function ff(a,b){a.buffer=ma(new a.ArrayType(b),a.buffer)}function gf(a,b){if(0<b){a.length+=b;b=a.length*a.stride;const c=a.buffer.length;b>=c&&ff(a,0===c?df(1*b,a.BYTES_PER_ELEMENT):df(2*b,a.BYTES_PER_ELEMENT))}return a}
class hf{constructor(a,b=1){this.buffer=a;this.stride=b;this.BYTES_PER_ELEMENT=a.BYTES_PER_ELEMENT;this.ArrayType=a.constructor;ff(this,this.length=Math.trunc(a.length/b))}get byteLength(){return Math.trunc(this.length*this.stride*this.BYTES_PER_ELEMENT)}get reservedLength(){return this.buffer.length/this.stride}get reservedByteLength(){return this.buffer.byteLength}set(){return this}append(a){return this.set(this.length,a)}flush(a=this.length){a=df(a*this.stride,this.BYTES_PER_ELEMENT);a=ef(this.buffer,
...
new jb);if(g+b===a.length)return new lb;if(k+b===a.length)return new Hb;if(n+b===a.length)return new Ub;if(c+b===a.length){const p=ri(a[a.findIndex(x=>null!=x)]);if(a.every(x=>null==x||pi(p,ri(x))))return new qc(new S("",p,!0))}else if(d+b===a.length){b=new Map;for(const p of a)for(const x of Object.keys(p))b.has(x)||null==p[x]||b.set(x,new S(x,ri([p[x]]),!0));return new C([...b.values()])}throw new TypeError("Unable to infer Vector type from input values, explicit type declaration expected");}
function si(a){const {queueingStrategy:b="count"}=a,{highWaterMark:c="bytes"!==b?Number.POSITIVE_INFINITY:16384}=a,d="bytes"!==b?"length":"byteLength";return function*(e){let f=0;const g=qh(a);for(const k of e)g.append(k)[d]>=c&&++f&&(yield g.toVector());if(0<g.finish().length||0===f)yield g.toVector()}};function ti(a,b){return ui(a,b.map(c=>c.data.concat()))}function ui(a,b){const c=[...a.fields],d=[],e={Xa:b.reduce((Sa,kb)=>Math.max(Sa,kb.length),0)};let f=0,g=0,k=-1;const n=b.length;let p,x=[];for(;0<e.Xa--;){g=Number.POSITIVE_INFINITY;for(k=-1;++k<n;)x[k]=p=b[k].shift(),g=Math.min(g,p?p.length:g);Number.isFinite(g)&&(x=vi(c,g,x,b,e),0<g&&(d[f++]=I({type:new C(c),length:g,nullCount:0,children:x.slice()})))}return[a=a.assign(c),d.map(Sa=>new W(a,Sa))]}
...
Z.Float32=vb;Z.Float32Builder=vh;Z.Float64=wb;Z.Float64Builder=wh;Z.FloatBuilder=th;Z.Int=B;Z.Int16=ib;Z.Int16Builder=Ch;Z.Int32=jb;Z.Int32Builder=Dh;Z.Int64=lb;Z.Int64Builder=Eh;Z.Int8=hb;Z.Int8Builder=Bh;Z.IntBuilder=Ah;Z.Interval=kc;Z.IntervalBuilder=xh;Z.IntervalDayTime=oc;Z.IntervalDayTimeBuilder=yh;Z.IntervalUnit=Ja;Z.IntervalYearMonth=pc;Z.IntervalYearMonthBuilder=zh;Z.JSONMessageReader=Mj;Z.List=qc;Z.ListBuilder=Jh;Z.MapBuilder=Kh;Z.MapRow=ee;Z.Map_=Lc;Z.Message=kj;Z.MessageHeader=Vi;
